...
namespace FairyGUI
{
    ...
    public class DisplayObject : EventDispatcher
    {
        ...
        private Dictionary<string, float> paramDic;


        public void SetShaderParam(Dictionary<string, string> param)
        {
            if (paramDic == null)
                paramDic = new Dictionary<string, float>();
            else
                paramDic.Clear();
            foreach (var item in param)
            {
                ...
                // set shader param
                string _paramKey = item.Key.Replace("shader", "");
                if (string.IsNullOrEmpty(_paramKey))
                    continue;
                float _paramValue = float.Parse(item.Value);
                paramDic.Add(_paramKey, _paramValue);
            }
        }
        ...
        private void SetMaterialParam()
        {
            ...
            {
                graphics.material.SetFloat(item.Key, item.Value);
            }
        }
