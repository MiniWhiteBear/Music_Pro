...
namespace FairyGUI
{
    ...
    public class Transition : ITweenListener
    {
        ...
        public void PlayBGM()
        {
            int _itemLen = _items.Length;
            for (int i = 0; i < _itemLen; i++)
            {
                TransitionItem _item = _items[i];
                if (_item.type != TransitionActionType.Sound)
                    continue;

                TValue_Sound _sound = _item.value as TValue_Sound;
                if (_sound.audioClip == null)
                {
                    if (UIConfig.soundLoader == null || _sound.sound.StartsWith(UIPackage.URL_PREFIX))
                    {
                        _sound.audioClip = UIPackage.GetItemAssetByURL(_sound.sound) as NAudioClip;

                        string _str = string.Empty;
                        if (_owner.UserData.TryGetValue("bgmlooptime",out _str))
                            _sound.audioClip.SetLoopTime(float.Parse(_str));
                    }
                    else
                    {
                        _sound.audioClip = UIConfig.soundLoader(_sound.sound);
                    }
                }

                if (_sound.audioClip != null && _sound.audioClip.nativeClip != null)
                    _owner.PlayBGM(_sound.audioClip, _sound.volume);
            }
        }
            ...
            switch (item.type)
            {
                ...
                case TransitionActionType.Sound:
                    if (_playing && item.time >= _startTime)
                    {
                        ...
                        if (value.audioClip != null && value.audioClip.nativeClip != null)
                            _owner.PlaySound(value.audioClip, value.volume);
                    }
                    break;
