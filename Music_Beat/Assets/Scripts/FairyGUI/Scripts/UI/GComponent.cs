...
namespace FairyGUI
{
    ...
    public class GComponent : GObject
    {
        ...
        public override Dictionary<string, string> UserData
        {
            get
            {
                // check local data
                ...
                if (string.IsNullOrEmpty(baseUserData) && data == null)
                    return userData;

                // get data                
                string _strRes = string.Empty;
                if (!string.IsNullOrEmpty(baseUserData))
                    _strRes = baseUserData;
                else if (data != null)
                    _strRes = (string)data;
        ...
        public void SetChildTable(LuaTable table = null)
        {
            if (childTable != null)
                return;

            childTable = table == null ? LuaClient.Instance.luaState.NewTable() : table;
        }

        [NoToLua]
        public void GetObjectInComponent(string head, LuaTable table = null)
        {
            for (int i = 0; i < numChildren; i++)
            {
                FairyGUI.GObject gObject = GetChildAt(i);

                // check object type ,if GComponent ,get child.
                string _name = string.IsNullOrEmpty(head) ? gObject.name : head + gObject.name;
                if (table != null)
                {
                    table.RawSet(_name, gObject);
                }
                else
                {
                    SetChildTable();
                    childTable.RawSet(_name, gObject);
                }

                if (gObject is GComponent)
                {
                    GComponent _component = gObject as GComponent;
                    string _head = head + _component.name + "_";
                    if (table != null)
                        _component.GetObjectInComponent(_head, table);
                    else
                        _component.GetObjectInComponent(_head, childTable);
                }
            }
        }
