...
namespace FairyGUI
{
    ...
    public class GList : GComponent
    {
        ...
        public GComponent AddItem()
        {
            GObject obj = GetFromPool(null);

            GComponent _child = AddChild(obj) as GComponent;
            _child.GetObjectInComponent(string.Empty);
            return _child;
        }

        public GComponent AddItem(LuaTable luaTable)
        {
            GObject obj = GetFromPool(null);

            GComponent _child = AddChild(obj) as GComponent;
            _child.SetChildTable(luaTable);
            _child.GetObjectInComponent(string.Empty, luaTable);
            return null;
        }

        public GComponent AddItem(string url)
        {
            GObject obj = GetFromPool(url);
