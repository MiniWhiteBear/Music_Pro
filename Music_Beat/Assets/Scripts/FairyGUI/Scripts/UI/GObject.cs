...
namespace FairyGUI
{
    public class GObject : EventDispatcher
    {
        ...
        public virtual LuaTable childTable
        {
            get;
            set;
        }
        ...
        public void GetChildTable()
        {
            ...
            GComponent _com = this as GComponent;
            _com.GetObjectInComponent(string.Empty);
        }
        ...
        public virtual Dictionary<string, string> UserData
        {
            get
            {
                // check local data
                ...
                // get data                
                string _strRes = (string)data;

                string[] _strArr = _strRes.Split(';');
                for (int i = 0; i < _strArr.Length; i++)
                {
                    string[] _strValue = _strArr[i].Split('=');
                    userData.Add(_strValue[0], _strValue[1]);
                }
        ...
        private TwoLoopingMusic _playTool;
        ...
        public void GetAudioSource()
        {
            if (audioSource == null)
            {
                audioSource = displayObject.gameObject.AddComponent<AudioSource>();
                audioSource.bypassEffects = true;
                audioSource.playOnAwake = false;
            }
        }

        public void GetBGMAudio()
        {
            if (bgmAudio == null)
            {
                bgmAudio = Stage.inst.gameObject.GetComponent<AudioSource>();
                AudioMixer _audioMixer = Stage.audioMixer;
                bgmAudio.outputAudioMixerGroup = _audioMixer.FindMatchingGroups("BGM")[0];
                bgmAudio.bypassEffects = true;
                bgmAudio.playOnAwake = false;
            }
        }

        public void PlayBGM(NAudioClip clip)
        {
            PlayBGM(clip, 1);
        }

        public void PlayBGM(NAudioClip clip, float volumeScale)
        {
            // get bgm audio source
            GetBGMAudio();

            // get tailloopingmusic
            if (_playTool == null)
                _playTool = Stage.audioTool;
            //_playTool.Audio = bgmAudio;

            // check is playing bgm
            if (bgmAudio.clip != null)
            {
                if (clip.nativeClip == bgmAudio.clip || _playTool.SourceClip == clip.nativeClip)
                    return;
                else
                    StopBGM(clip.nativeClip);
            }

            if (clip.nativeClip != null && Stage.inst.soundVolume > 0)
            {
                bgmAudio.clip = clip.nativeClip;
                if (clip.loopTime >= 0)
                {
                    bgmAudio.volume = Stage.inst.soundVolume * volumeScale;
                    _playTool.Init(clip.nativeClip, clip.loopTime, true);
                    _playTool.Play();
                }
                else
                {
                    Debug.LogError("UI BGM looptime error, audioClip: " + clip.nativeClip.name);
                }
            }
        }
        ...
        public void PlaySound(NAudioClip clip, float volumeScale)
        {
            // get bgm audio source
            GetAudioSource();
            if (audioSource != null && Stage.inst.soundVolume > 0)
            {
                audioSource.clip = clip.nativeClip;
                audioSource.PlayOneShot(clip.nativeClip, volumeScale * Stage.inst.soundVolume);
            }
        }
        ...
        public void PlaySound(AudioClip clip, float volumeScale)
        {
            ...
            if (audioSource != null && Stage.inst.soundVolume > 0)
            {
                ...
                audioSource.PlayOneShot(clip, volumeScale * Stage.inst.soundVolume);
            }
        }
        ...
        public void PlaySound(AudioClip clip)
        {
            ...
            {
                audioSource.clip = clip;
                audioSource.PlayOneShot(clip, Stage.inst.soundVolume);
            }
        }
